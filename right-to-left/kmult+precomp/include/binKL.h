...
align void scalar_mult_fixed_base(unsigned char op[32], unsigned char n[32]){
	...
	vec 	vbit, t1;
	gfe1x 	r0x,r1x,r1z,r2x,r2z;
	gfe1x 	t2,t3,t4,t22;
	gfe 	op_gfe;

	

	r0x.v[0] = precompBase[1][0]; r0x.v[1] = precompBase[1][1]; r0x.v[2] = precompBase[1][2]; r0x.v[3] = precompBase[1][3];
	
	r1x.v[0] = one;     r1x.v[1]=zero; r1x.v[2]=zero; r1x.v[3]=zero;
	r1z.v[0] = zero;    r1z.v[1]=zero; r1z.v[2]=zero; r1z.v[3]=zero;

	r2x.v[0] = precompBase[1][0]; r2x.v[1] = precompBase[1][1]; r2x.v[2] = precompBase[1][2]; r2x.v[3] = precompBase[1][3];
	...
	for(i=0;i<31;i++){
			...
			ladderStep(&r0x, &r1x, &r1z, &r2x, &r2z);

        	        r0x.v[0] = precompBase[k][0]; r0x.v[1] = precompBase[k][1]; r0x.v[2] = precompBase[k][2]; r0x.v[3] = precompBase[k][3];
        	        k++;
